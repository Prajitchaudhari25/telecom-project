...
def render_expansion(df: pd.DataFrame):
    ...
    with c1:
        ...
        st.dataframe(
            roi_tbl.style.applymap(style_priority, subset=["priority"]).format(
                {
                    "roi": "{:.1f}%",
                    "demand_score": "{:.3f}",
                    "expansion_cost": "₹{:,.0f}",
    ...
    with c2:
        ...
        st.dataframe(
            dem_tbl.style.applymap(style_priority, subset=["priority"]).format(
                {
                    "demand_score": "{:.3f}",
                    "roi": "{:.1f}%",
                    "readiness_score": "{:.2f}",
...
def render_scenario(df: pd.DataFrame, df_raw: pd.DataFrame, demand_adj: float, roi_adj: float):
    ...
    if changed.empty:
        ...
        st.dataframe(
            changed.style.applymap(style_priority, subset=["baseline_priority", "scenario_priority"]),
            use_container_width=True,
        )
...
def render_recommendations(ai_bundle: dict):
    ...
    st.dataframe(
        top_rec.style.applymap(style_priority, subset=["priority"]).format(
            {
                "pred_roi": "{:.1f}%",
                "estimated_uplift": "₹{:,.0f}",
                "confidence": "{:.0%}",
