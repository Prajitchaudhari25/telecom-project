"""
tabs.py (FIXED FOR STREAMLIT CLOUD)
"""
...
# ───────────────── MAP ─────────────────
def render_map(df):
    section_header("🗺️", "Map")

    size_col = st.selectbox("Bubble size", ["population", "roi", "demand_score"])
...
def render_explainability(ai_bundle):
    ...
    st.plotly_chart(
        px.bar(imp, x="importance", y="feature"),
        use_container_width=True
    )
